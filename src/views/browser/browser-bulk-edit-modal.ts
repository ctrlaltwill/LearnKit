...
import {
  attachClozeShortcuts, createMobileClozeButtons, shouldShowMobileClozeButtons, } from "../../platform/card-editor/card-editor";
...
import {
  clearNode, titleCaseGroupPath, formatGroupDisplay, expandGroupAncestors, parseGroupsInput, sortGroupPathsForDisplay, groupsToInput, } from "./browser-helpers";
...
export class BulkEditModal extends Modal {
  ...
  onOpen() {
    ...
    const handleModalPointerDownBlur = (ev: PointerEvent) => {
      ...
      const activeEl = active as HTMLInputElement | HTMLTextAreaElement;
      if (!activeEl.classList.contains("learnkit-flag-editor-control")) return;
      const activeWrap = activeEl.closest<HTMLElement>(".learnkit-flag-editor-wrap");
      if (activeWrap?.contains(target)) return;
      activeEl.blur();
    };
    activeDocument.addEventListener("pointerdown", handleModalPointerDownBlur, true);
    registerCloseCleanup(() => {
      activeDocument.removeEventListener("pointerdown", handleModalPointerDownBlur, true);
    });
  ...
  const attachFlagPreviewOverlay = (
    ...
    wrap.className = `learnkit-flag-editor-wrap${control.tagName === "TEXTAREA" ? " learnkit-flag-editor-wrap--multiline" : ""}`;

    const overlay = activeDocument.createElement("div");
    overlay.className = `learnkit-flag-editor-overlay${control.tagName === "TEXTAREA" ? " learnkit-flag-editor-overlay--multiline" : ""}`;
    ...
    if (typeof ResizeObserver !== "undefined") {
      const ro = new ResizeObserver(() => {
        if (control.tagName === "TEXTAREA" && wrap.classList.contains("learnkit-flag-editor--focused")) {
  ...
  const createFieldWrapper = (field: { key: ColKey; label: string; editable: boolean }) => {
    ...
    if (field.key === "type" && canBulkToggleType) {
      const typeRoot = activeDocument.createElement("div");
      typeRoot.className = "learnkit relative inline-flex";

      const typeButton = activeDocument.createElement("button");
      ...
      const typeText = activeDocument.createElement("span");
      ...
      const makeTypeItem = (type: "basic" | "reversed", labelText: string) => {
        ...
        const radioWrap = activeDocument.createElement("div");
        ...
        const apply = () => {
          ...
          for (const child of Array.from(menuList.children)) {
            if (child.nodeType === Node.ELEMENT_NODE) {
              ...
              const isChecked = childEl === item;
              childEl.setAttribute("aria-checked", isChecked ? "true" : "false");
              const marker = childEl.querySelector<HTMLElement>(".size-2");
    ...
    if (field.key === "question" && input.tagName === "TEXTAREA" && isClozeOnly) {
