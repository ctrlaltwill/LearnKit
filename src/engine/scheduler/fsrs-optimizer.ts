...
/** A single review in a card's history, ready for training. */
interface TrainingReview {
  rating: number; // 1-4
  deltaDays: number; // days since previous review (0 for first)
}

/** One card's full review sequence. */
interface TrainingItem {
  reviews: TrainingReview[];
}

/** Progress callback for the UI. */
export type OptimiserProgress = (pct: number) => void;
 ...
 */
function preprocessReviewLogs(logs: ReviewLogEntry[]): TrainingItem[] {
  // Group by card id
  ...
  for (const [, entries] of byCard) {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      const deltaDays =
        i === 0 ? 0 : Math.max(0, (entries[i].at - entries[i - 1].at) / MS_PER_DAY);
...
function ratingToNumber(result: string): number {
  switch (result) {
    case "again":
    case "fail":
 ...
 */
function computeLoss(w: number[], items: TrainingItem[]): number {
...
function runAdam(
  ...
  for (let epoch = 0; epoch < MAX_EPOCHS; epoch++) {
    ...
    if (loss < bestLoss) {
      ...
      staleCount++;
      if (staleCount > 12) break; // early stopping
    }
    ...
    onProgress?.(Math.round(((epoch + 1) / MAX_EPOCHS) * 100));
  }
